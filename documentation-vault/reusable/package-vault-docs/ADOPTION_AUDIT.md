...
| `./PackagesForReuse/AppFormValidation` | connected | Used source-only through `./PackagesInUse/AppFormValidation` for login form validation mechanics while localized copy, password policy, auth callbacks, and error presentation remain app-owned. |
...
| `./PackagesForReuse/AppValidationCore` | vault-only | Current app already uses `./PackagesInUse/AppFormValidation` for login form validation. `AppValidationCore` is a lower-level generic validation foundation for future settings/domain/payload validation, but connecting it now would create a parallel validation surface without a current app consumer. |
