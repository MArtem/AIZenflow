...
if grep -RInE 'String\(describing:[[:space:]]*error\)|stablePrivacyHash|SecureStorageKey\.init' \
  "$PACKAGE_ROOT/Sources" "$PACKAGE_ROOT/Tests" 2>/dev/null; then
  fail "Forbidden secure-storage diagnostic or ambiguous initializer pattern found"
fi
