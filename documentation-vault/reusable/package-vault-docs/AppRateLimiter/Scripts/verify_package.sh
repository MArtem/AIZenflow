...
[[ "${PACKAGE_NAME}" == "AppRateLimiter" ]] || fail "package folder must be AppRateLimiter"
...
if grep -E "warning:|error:" "${NORMAL_LOG}" "${STRICT_LOG}" >/dev/null; then
...
echo "✅ AppRateLimiter verification passed"
