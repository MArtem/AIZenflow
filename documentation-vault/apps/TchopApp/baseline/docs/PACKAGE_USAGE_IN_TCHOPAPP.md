...
The active package source files must appear in the Xcode project under a logical `PackagesInUse` group with one subgroup per package. New packages added to `./PackagesInUse` must be added to the project with the same logical grouping, not left in `Recovered References`. Physical source layout remains unchanged under `./PackagesInUse/<PackageName>`.
