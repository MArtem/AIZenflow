...
- [ ] 8.2: провести сложный multi-target consumer pilot; German boundary, QC pin, семь clean-snapshot adapters, six-scheme Debug builds, app smoke launch, positive/negative QC fixtures и reversible bootstrap PASS; authorized test-source repair применён, unit tests и все 7 UI tests PASS, bounded host fixture activation/lifecycle/accessibility contract PASS_WITH_LIMITATION, GitHub parity PASS после merge PR #23, QC PR #25 merged with independent review, bounded doctor effective-settings PASS, а graph-scoped static-evidence, source-membership evidence, physical-device VoiceOver traversal и pre-PR review ещё не завершены.
