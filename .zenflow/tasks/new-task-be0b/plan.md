...
task-created DerivedData after evidence is captured. Tests remain out of scope. Commit and push
authority was subsequently granted for the corresponding development/main (or sole default) branches;
PR authority remains absent.
...
- [x] Review exact diffs, record a compact receipt with omissions/residual risk, remove only
  task-created DerivedData/logs, commit the migration, and publish it while preserving user-owned changes.
