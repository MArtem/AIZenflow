...
2. Зафиксируй инварианты и failure modes именно для задачи: **задать SLO для launch, interaction, scroll, memory, network и regression gate**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-11-11: «Performance budget».
...
Особый фокус: задать SLO для launch, interaction, scroll, memory, network и regression gate.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-11-11.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.

## Review-ready depth — performance evidence

**Scenario.** A change is proposed as a performance optimization without a measured user-visible bottleneck.

**Common wrong approach.** Optimize based on code aesthetics, a single debug run, or simulator timing and then claim a production win.

**Preferred approach.** Define the metric and scenario first, collect a baseline under controlled conditions, change one meaningful variable, and compare the same metric/scenario. Preserve regressions with a repeatable performance test when feasible.

**Traps / edge cases.** Debug-vs-release differences, simulator/device divergence, thermal state, cache warmup, network variance, percentile effects, memory/CPU tradeoffs, and optimizations that improve average time while worsening responsiveness or energy.

**Verification.** Instruments/Xcode metrics for diagnosis, XCTest performance baselines for repeatable local regression signals, and shipping metrics when the problem is production-only.

Primary sources checked 2026-09-10: https://developer.apple.com/documentation/xcode/performance-and-metrics and https://developer.apple.com/documentation/xcode/writing-and-running-performance-tests

**Compatibility.** Compare performance under the app's actual minimum/representative OS versions, devices, build configuration, and toolchain. A metric or instrument unavailable on an older supported environment must not become the only acceptance signal for that environment.
