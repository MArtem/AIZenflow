...
2. Зафиксируй инварианты и failure modes именно для задачи: **threat model → trust boundaries → inspect secrets/auth/network/webviews/deeplinks/logging → mitigations**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-07: «Security audit workflow».
...
Особый фокус: threat model → trust boundaries → inspect secrets/auth/network/webviews/deeplinks/logging → mitigations.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-07.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
