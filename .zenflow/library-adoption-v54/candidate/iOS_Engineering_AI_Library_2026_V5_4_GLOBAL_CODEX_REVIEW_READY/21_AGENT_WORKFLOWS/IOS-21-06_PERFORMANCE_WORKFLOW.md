...
2. Зафиксируй инварианты и failure modes именно для задачи: **define metric → measure baseline → find bottleneck → patch → compare → regression guard**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-06: «Performance optimization workflow».
...
Особый фокус: define metric → measure baseline → find bottleneck → patch → compare → regression guard.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-06.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
