...
2. Зафиксируй инварианты и failure modes именно для задачи: **reproduce → hypothesis → evidence → minimal fix → regression test → risk scan**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-02: «Bugfix workflow».
...
Особый фокус: reproduce → hypothesis → evidence → minimal fix → regression test → risk scan.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-02.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
