...
2. Зафиксируй инварианты и failure modes именно для задачи: **inspect diff + surrounding code + tests + architecture; produce severity-ranked findings with fixes**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-03: «Deep review workflow».
...
Особый фокус: inspect diff + surrounding code + tests + architecture; produce severity-ranked findings with fixes.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-03.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
