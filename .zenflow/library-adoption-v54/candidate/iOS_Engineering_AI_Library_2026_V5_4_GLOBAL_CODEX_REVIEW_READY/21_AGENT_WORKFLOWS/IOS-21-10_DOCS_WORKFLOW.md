...
domain: AI Coding Agent Workflows
priority: production
...
2. Зафиксируй инварианты и failure modes именно для задачи: **derive docs from code/contracts, state invariants/decisions/examples/failure modes and keep update triggers**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-10: «Technical documentation workflow».
...
Особый фокус: derive docs from code/contracts, state invariants/decisions/examples/failure modes and keep update triggers.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-10.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
