...
2. Зафиксируй инварианты и failure modes именно для задачи: **repo inspection → plan → incremental implementation → tests → build → review summary**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-01: «Feature implementation workflow».
...
Особый фокус: repo inspection → plan → incremental implementation → tests → build → review summary.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-01.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
