...
2. Зафиксируй инварианты и failure modes именно для задачи: **inventory → compatibility matrix → seam strategy → dual-run/bridge → tests → rollout → cleanup**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-08: «Migration workflow».
...
Особый фокус: inventory → compatibility matrix → seam strategy → dual-run/bridge → tests → rollout → cleanup.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-08.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
