...
2. Зафиксируй инварианты и failure modes именно для задачи: **behavior lock → dependency map → small steps → tests/build after each logical slice**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-21-04: «Refactor workflow».
...
Особый фокус: behavior lock → dependency map → small steps → tests/build after each logical slice.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-21-04.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
