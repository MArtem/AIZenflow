...
5. Проверь: typed config, defaults, stale config, rollout, analytics, cleanup.
...
## Готовый prompt
```text
Примени skill IOS-25-07: «Remote config» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: typed keys/defaults, cache/TTL, failure semantics, experiment ownership, kill switches and cleanup.
Проверь: typed config, defaults, stale config, rollout, analytics, cleanup.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-25-07.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
