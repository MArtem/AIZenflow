...
5. Проверь: schema governance, consent, PII, identity, retry, environment separation.
...
## Готовый prompt
```text
Примени skill IOS-25-06: «Analytics SDK» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: event schema, PII prohibition, consent, batching, retry, identity reset and test/debug mode.
Проверь: schema governance, consent, PII, identity, retry, environment separation.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-25-06.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
