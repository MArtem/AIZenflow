...
## Назначение
Метрики, warmup, baseline, variance и CI signal без flaky thresholds.
...
2. Зафиксируй инварианты и failure modes именно для задачи: **метрики, warmup, baseline, variance и CI signal без flaky thresholds**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-10-10: «Performance regression tests».
...
Особый фокус: метрики, warmup, baseline, variance и CI signal без flaky thresholds.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-10-10.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
