...
## Назначение
Генерировать edge inputs для parsers/state machines/serialization и сохранять reproducibility.
...
2. Зафиксируй инварианты и failure modes именно для задачи: **генерировать edge inputs для parsers/state machines/serialization и сохранять reproducibility**.
...
5. Проверь тематические quality gates: determinism, failure paths, parallel safety, fixtures, signal vs brittleness.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-10-09: «Property/fuzz tests».
...
Особый фокус: генерировать edge inputs для parsers/state machines/serialization и сохранять reproducibility.
Проверь: determinism, failure paths, parallel safety, fixtures, signal vs brittleness.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-10-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
