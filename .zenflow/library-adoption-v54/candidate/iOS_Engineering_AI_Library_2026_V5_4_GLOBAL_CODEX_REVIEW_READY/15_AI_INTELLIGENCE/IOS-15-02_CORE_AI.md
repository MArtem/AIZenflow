...
id: IOS-15-02
type: skill
title: Core AI/on-device models
domain: AI / Intelligence
priority: production
baseline: Swift 6 / modern iOS
---

# Core AI/on-device models

## Назначение
Model packaging, memory budget, device specialization, stateful inference, latency и fallback.
...
2. Зафиксируй инварианты и failure modes именно для задачи: **model packaging, memory budget, device specialization, stateful inference, latency и fallback**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-15-02: «Core AI/on-device models».
...
Особый фокус: model packaging, memory budget, device specialization, stateful inference, latency и fallback.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-15-02.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
