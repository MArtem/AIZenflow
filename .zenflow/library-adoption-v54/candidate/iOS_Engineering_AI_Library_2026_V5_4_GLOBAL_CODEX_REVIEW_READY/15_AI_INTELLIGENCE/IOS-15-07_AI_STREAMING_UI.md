...
## Назначение
Partial state, cancellation, reconnect, token/segment rendering, accessibility и error recovery.
...
2. Зафиксируй инварианты и failure modes именно для задачи: **partial state, cancellation, reconnect, token/segment rendering, accessibility и error recovery**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-15-07: «Streaming AI UI».
...
Особый фокус: partial state, cancellation, reconnect, token/segment rendering, accessibility и error recovery.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-15-07.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
