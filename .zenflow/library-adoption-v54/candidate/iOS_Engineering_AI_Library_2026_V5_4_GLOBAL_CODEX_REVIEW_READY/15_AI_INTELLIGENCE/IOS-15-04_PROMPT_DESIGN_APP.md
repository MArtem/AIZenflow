...
## Назначение
System/user/tool boundaries, structured outputs, grounding, refusal/fallback UX и prompt injection awareness.
...
2. Зафиксируй инварианты и failure modes именно для задачи: **system/user/tool boundaries, structured outputs, grounding, refusal/fallback UX и prompt injection awareness**.
...
5. Проверь тематические quality gates: capability gating, evaluation, privacy, tool authorization, fallback, latency.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-15-04: «In-app prompt design».
...
Особый фокус: system/user/tool boundaries, structured outputs, grounding, refusal/fallback UX и prompt injection awareness.
Проверь: capability gating, evaluation, privacy, tool authorization, fallback, latency.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-15-04.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
