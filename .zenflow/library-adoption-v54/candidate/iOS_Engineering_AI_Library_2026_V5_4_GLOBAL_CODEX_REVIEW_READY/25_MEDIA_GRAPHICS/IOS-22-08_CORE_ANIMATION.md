...
5. Проверь: model/presentation state, completion/cancellation, offscreen cost, rasterization, Reduce Motion.
...
## Готовый prompt
```text
Примени skill IOS-22-08: «Core Animation» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: делать layer-backed animation, transactions, presentation/model layers, timing и Reduce Motion.
Проверь: model/presentation state, completion/cancellation, offscreen cost, rasterization, Reduce Motion.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-08.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
