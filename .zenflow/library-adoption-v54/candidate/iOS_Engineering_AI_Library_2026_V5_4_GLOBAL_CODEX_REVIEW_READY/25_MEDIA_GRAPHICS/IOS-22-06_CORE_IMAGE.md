...
## Назначение
Строить filter/render pipeline, CIContext reuse, color spaces, GPU/CPU choice и image orientation.
...
5. Проверь: CIContext lifetime, color space, extent, GPU memory, thread safety, measurement.
...
## Готовый prompt
```text
Примени skill IOS-22-06: «Core Image pipeline» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: строить filter/render pipeline, CIContext reuse, color spaces, GPU/CPU choice и image orientation.
Проверь: CIContext lifetime, color space, extent, GPU memory, thread safety, measurement.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-06.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
