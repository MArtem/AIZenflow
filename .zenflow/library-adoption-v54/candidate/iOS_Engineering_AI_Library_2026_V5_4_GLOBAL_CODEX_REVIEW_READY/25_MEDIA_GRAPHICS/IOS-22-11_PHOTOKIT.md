...
## Назначение
Limited library, fetch/change observers, authorization changes, asset requests, iCloud-backed assets и cancellation.
...
5. Проверь: limited permission, request cancellation, iCloud latency, identifiers, privacy, memory.
...
## Готовый prompt
```text
Примени skill IOS-22-11: «PhotoKit» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: limited library, fetch/change observers, authorization changes, asset requests, iCloud-backed assets и cancellation.
Проверь: limited permission, request cancellation, iCloud latency, identifiers, privacy, memory.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-11.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
