...
5. Проверь: route/interruption state, activation policy, background, Bluetooth/AirPlay, user expectations.
...
## Готовый prompt
```text
Примени skill IOS-22-02: «AVAudioSession» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: корректно выбрать category/mode/options, route changes, interruptions, background audio и coexistence с другими apps.
Проверь: route/interruption state, activation policy, background, Bluetooth/AirPlay, user expectations.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-02.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
