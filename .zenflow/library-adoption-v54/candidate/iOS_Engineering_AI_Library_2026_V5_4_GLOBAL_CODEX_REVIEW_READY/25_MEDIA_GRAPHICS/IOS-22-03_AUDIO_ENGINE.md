...
5. Проверь: realtime safety, allocations/locks, formats, route changes, latency, teardown.
...
## Готовый prompt
```text
Примени skill IOS-22-03: «AVAudioEngine» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: построить audio graph, format conversion, realtime-thread safety, interruption recovery и latency.
Проверь: realtime safety, allocations/locks, formats, route changes, latency, teardown.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-03.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
