...
5. Проверь: GPU synchronization, resource hazards, memory, frame pacing, thermal, fallback.
...
## Готовый prompt
```text
Примени skill IOS-22-09: «Metal rendering/compute» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: command queues/buffers, resource lifetime, synchronization, pipeline caching, memory heaps и GPU capture.
Проверь: GPU synchronization, resource hazards, memory, frame pacing, thermal, fallback.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-22-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
