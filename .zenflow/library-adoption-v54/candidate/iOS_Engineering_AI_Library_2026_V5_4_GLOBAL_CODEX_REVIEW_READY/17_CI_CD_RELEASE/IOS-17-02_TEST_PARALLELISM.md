...
2. Зафиксируй инварианты и failure modes именно для задачи: **sharding, simulator allocation, shared-state isolation, retries only as diagnostics и result merging**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-17-02: «CI test parallelism».
...
Особый фокус: sharding, simulator allocation, shared-state isolation, retries only as diagnostics и result merging.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-17-02.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
