...
2. Зафиксируй инварианты и failure modes именно для задачи: **применять wrapper для повторяемой property semantics без скрытого lifecycle/side effects**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-02-09: «Property wrappers».
...
Особый фокус: применять wrapper для повторяемой property semantics без скрытого lifecycle/side effects.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-02-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
