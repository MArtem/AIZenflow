...
id: OP-IOS-06-09
type: deep-playbook
source_skill: IOS-06-09
title: Постепенная миграция UIKit→SwiftUI — Operational Deep Playbook
section: 06_UIKIT
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-06-09 — Постепенная миграция UIKit→SwiftUI

## Mission
выбрать seams, hosting boundaries, shared models и rollback path без big-bang rewrite

Этот playbook — operational-версия `06_UIKIT/IOS-06-09_UIKIT_SWIFTUI_MIGRATION.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Постепенная миграция UIKit→SwiftUI».
...
- For **Постепенная миграция UIKit→SwiftUI**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-06-09 «Постепенная миграция UIKit→SwiftUI» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: выбрать seams, hosting boundaries, shared models и rollback path без big-bang rewrite.
