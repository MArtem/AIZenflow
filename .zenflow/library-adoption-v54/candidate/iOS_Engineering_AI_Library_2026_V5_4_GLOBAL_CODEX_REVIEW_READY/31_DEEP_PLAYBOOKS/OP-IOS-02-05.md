...
id: OP-IOS-02-05
type: deep-playbook
source_skill: IOS-02-05
title: Enum/state machine modeling — Operational Deep Playbook
section: 02_SWIFT_LANGUAGE
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-02-05 — Enum/state machine modeling

## Mission
закодировать допустимые состояния и переходы в типах, исключая impossible states

Этот playbook — operational-версия `02_SWIFT_LANGUAGE/IOS-02-05_ENUM_STATE_MACHINE.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Enum/state machine modeling».
...
- For **Enum/state machine modeling**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-02-05 «Enum/state machine modeling» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: закодировать допустимые состояния и переходы в типах, исключая impossible states.
