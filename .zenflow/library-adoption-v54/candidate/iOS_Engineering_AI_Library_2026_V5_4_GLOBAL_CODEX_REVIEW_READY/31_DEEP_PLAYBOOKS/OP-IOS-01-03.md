...
id: OP-IOS-01-03
type: deep-playbook
source_skill: IOS-01-03
title: Разбор backend/API контракта — Operational Deep Playbook
section: 01_DISCOVERY_PRODUCT
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-01-03 — Разбор backend/API контракта

## Mission
проверить schema, optionality, enums, versioning, pagination, errors, idempotency, timestamps/timezones, backward compatibility

Этот playbook — operational-версия `01_DISCOVERY_PRODUCT/IOS-01-03_API_CONTRACT_REVIEW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Разбор backend/API контракта».
...
- Verify cursor invalidation, deduplication, refresh-vs-load-more races, stable IDs and partial failure recovery.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-01-03 «Разбор backend/API контракта» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: проверить schema, optionality, enums, versioning, pagination, errors, idempotency, timestamps/timezones, backward compatibility.
