...
id: OP-IOS-02-03
type: deep-playbook
source_skill: IOS-02-03
title: Generics и protocols — Operational Deep Playbook
section: 02_SWIFT_LANGUAGE
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-02-03 — Generics и protocols

## Mission
использовать generics/existentials/opaque types/PAT только там, где это улучшает type safety и abstraction boundary

Этот playbook — operational-версия `02_SWIFT_LANGUAGE/IOS-02-03_GENERICS_PROTOCOLS.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- existing public/internal API conventions
- call sites and generic constraints
- Swift language/compiler version and upcoming features
- Objective-C/C exposure and package boundaries
- benchmarks only when performance motivates language-level complexity
- Конкретные call sites/owners/tests, затронутые именно задачей «Generics и protocols».
- Deployment/availability/toolchain ограничения до предложения нового API.

## Invariants
- types make invalid states difficult to express
- ownership and mutation semantics are obvious
- public API compatibility is preserved when required
- errors are not hidden by type erasure or force operations
- advanced language features have measurable benefit

### Topic-specific invariants / traps
- For **Generics и protocols**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
- start from simplest concrete type-safe API
- prefer value semantics unless identity/shared mutable state is required
- use generics/protocols only at a real substitution boundary
- document ownership/borrowing/unsafe invariants close to code

## Mode B — REVIEW
Review не является пересказом diff. Ищи конкретный failure mode и подтверждай его surrounding code.

- find force unwrap/cast, Any/stringly contracts and accidental reference semantics
- inspect generic/protocol complexity versus actual callers
- check public API source compatibility and resilience assumptions
...
- reduce compiler/runtime problem to a minimal type/ownership example
- inspect generated interfaces or macro expansion when behavior is non-obvious
- verify Foundation locale/time-zone semantics with controlled fixtures

## Mode D — MIGRATION
Применяй `00_META/MIGRATION_PROTOCOL.md`.

- preserve source behavior and serialization/public contracts
- introduce language-feature migrations in narrow slices with compile-time tests
...
- target/package builds under declared Swift mode
- unit tests for boundary/edge cases
- API compatibility or symbol/interface check when public
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-02-03 «Generics и protocols» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: использовать generics/existentials/opaque types/PAT только там, где это улучшает type safety и abstraction boundary.
