...
id: OP-IOS-10-12
type: deep-playbook
source_skill: IOS-10-12
title: Deterministic time/random/UUID — Operational Deep Playbook
section: 10_TESTING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-10-12 — Deterministic time/random/UUID

## Mission
инъецировать controllable dependencies для воспроизводимых тестов

Этот playbook — operational-версия `10_TESTING/IOS-10-12_CLOCK_RANDOM_UUID.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Deterministic time/random/UUID».
...
- For **Deterministic time/random/UUID**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-10-12 «Deterministic time/random/UUID» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: инъецировать controllable dependencies для воспроизводимых тестов.
