...
id: OP-IOS-09-07
type: deep-playbook
source_skill: IOS-09-07
title: Offline-first data — Operational Deep Playbook
section: 09_PERSISTENCE_DATA
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-09-07 — Offline-first data

## Mission
operation log/sync state/conflicts/retries/idempotency и user-visible consistency

Этот playbook — operational-версия `09_PERSISTENCE_DATA/IOS-09-07_OFFLINE_FIRST.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Offline-first data».
...
- For **Offline-first data**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-09-07 «Offline-first data» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: operation log/sync state/conflicts/retries/idempotency и user-visible consistency.
