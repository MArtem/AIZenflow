...
id: OP-IOS-21-01
type: deep-playbook
source_skill: IOS-21-01
title: Feature implementation workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-01 — Feature implementation workflow

## Mission
repo inspection → plan → incremental implementation → tests → build → review summary

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-01_FEATURE_IMPLEMENTATION_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Feature implementation workflow».
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-01 «Feature implementation workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: repo inspection → plan → incremental implementation → tests → build → review summary.
