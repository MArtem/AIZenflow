...
id: OP-IOS-05-16
type: deep-playbook
source_skill: IOS-05-16
title: SwiftUI code review — Operational Deep Playbook
section: 05_SWIFTUI
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-05-16 — SwiftUI code review

## Mission
искать duplicated state, invalidation storms, identity hacks, side effects in body, async lifetime и accessibility gaps

Этот playbook — operational-версия `05_SWIFTUI/IOS-05-16_SWIFTUI_REVIEW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «SwiftUI code review».
...
- For **SwiftUI code review**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-05-16 «SwiftUI code review» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: искать duplicated state, invalidation storms, identity hacks, side effects in body, async lifetime и accessibility gaps.
