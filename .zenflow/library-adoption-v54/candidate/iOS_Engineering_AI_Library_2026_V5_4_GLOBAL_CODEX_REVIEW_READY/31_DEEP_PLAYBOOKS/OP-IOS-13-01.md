...
id: OP-IOS-13-01
type: deep-playbook
source_skill: IOS-13-01
title: VoiceOver — Operational Deep Playbook
section: 13_ACCESSIBILITY_LOCALIZATION
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-13-01 — VoiceOver

## Mission
semantic labels/values/hints/actions, grouping, focus order и dynamic updates

Этот playbook — operational-версия `13_ACCESSIBILITY_LOCALIZATION/IOS-13-01_VOICEOVER.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «VoiceOver».
...
- For **VoiceOver**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-13-01 «VoiceOver» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: semantic labels/values/hints/actions, grouping, focus order и dynamic updates.
