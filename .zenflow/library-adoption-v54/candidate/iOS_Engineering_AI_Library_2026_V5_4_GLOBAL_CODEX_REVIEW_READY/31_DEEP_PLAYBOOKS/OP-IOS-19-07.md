...
id: OP-IOS-19-07
type: deep-playbook
source_skill: IOS-19-07
title: Error handling refactor — Operational Deep Playbook
section: 19_CODE_REVIEW_REFACTOR
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-19-07 — Error handling refactor

## Mission
убрать swallowed errors, normalize mapping, recoverability, logging и user messaging

Этот playbook — operational-версия `19_CODE_REVIEW_REFACTOR/IOS-19-07_ERROR_REFACTOR.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Error handling refactor».
...
- For **Error handling refactor**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-19-07 «Error handling refactor» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: убрать swallowed errors, normalize mapping, recoverability, logging и user messaging.
