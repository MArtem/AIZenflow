...
id: OP-IOS-12-07
type: deep-playbook
source_skill: IOS-12-07
title: Permissions — Operational Deep Playbook
section: 12_SECURITY_PRIVACY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-12-07 — Permissions

## Mission
минимизация, just-in-time rationale, denied/restricted states и Settings recovery UX

Этот playbook — operational-версия `12_SECURITY_PRIVACY/IOS-12-07_PERMISSIONS.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- assets/PII/secrets and threat boundary
- authentication versus authorization checks
- Keychain/Secure Enclave/storage and token lifetime
- ATS/trust/deep-link/WebView boundaries
- PrivacyInfo.xcprivacy and third-party SDK data flows
- Конкретные call sites/owners/tests, затронутые именно задачей «Permissions».
- Deployment/availability/toolchain ограничения до предложения нового API.

## Invariants
- least privilege and least data collection
- authorization is enforced at trusted boundary
- secrets never live in source/UserDefaults/logs
- TLS/trust checks are never disabled as a workaround
- privacy manifest matches actual behavior

### Topic-specific invariants / traps
- For **Permissions**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
- model sensitive flows with explicit threat assumptions
- use Keychain and platform security mechanisms appropriately
- validate external inputs before privileged effects
- redact logs and separate analytics identity from sensitive payload

## Mode B — REVIEW
Review не является пересказом diff. Ищи конкретный failure mode и подтверждай его surrounding code.

- search for secrets, PII logging, broad permissions and unsafe WebView bridges
- check auth/logout races, deep-link privilege escalation and trust bypass
- audit third-party SDK privacy manifests/signatures/data collection
...
- preserve evidence without copying secrets into debug output
- reproduce with synthetic accounts/data when possible
- contain issue first if exposure/data corruption is ongoing

## Mode D — MIGRATION
Применяй `00_META/MIGRATION_PROTOCOL.md`.

- rotate credentials/keys/contracts safely with overlap where needed
- stage privacy/security changes and preserve revocation/rollback
...
- negative authorization/input tests
- privacy manifest/report review
- security boundary integration tests and release audit
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-12-07 «Permissions» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: минимизация, just-in-time rationale, denied/restricted states и Settings recovery UX.
