...
id: OP-IOS-17-08
type: deep-playbook
source_skill: IOS-17-08
title: Hotfix workflow — Operational Deep Playbook
section: 17_CI_CD_RELEASE
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-17-08 — Hotfix workflow

## Mission
minimal diff, cherry-pick safety, expedited validation, kill switch и postmortem follow-up

Этот playbook — operational-версия `17_CI_CD_RELEASE/IOS-17-08_HOTFIX.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Hotfix workflow».
...
- For **Hotfix workflow**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-17-08 «Hotfix workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: minimal diff, cherry-pick safety, expedited validation, kill switch и postmortem follow-up.
