...
id: OP-IOS-18-10
type: deep-playbook
source_skill: IOS-18-10
title: Debuggability review — Operational Deep Playbook
section: 18_OBSERVABILITY_DEBUGGING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-18-10 — Debuggability review

## Mission
достаточны ли logs/metrics/ids/state transitions для расследования без reproducing everything

Этот playbook — operational-версия `18_OBSERVABILITY_DEBUGGING/IOS-18-10_DEBUG_REVIEW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- crash/hang/race/network/data symptom and affected versions
- symbolication and build UUID/dSYM mapping
- OSLog categories, signposts, MetricKit/crash SDK telemetry
- reproduction environment/state/feature flags
- recent regression range and ownership
- Конкретные call sites/owners/tests, затронутые именно задачей «Debuggability review».
- Deployment/availability/toolchain ограничения до предложения нового API.

## Invariants
- root cause is distinguished from correlated symptom
- debug data contains enough context but no sensitive payload
- instrumentation changes behavior minimally
- fix has a regression detector/test when practical
- production containment precedes risky refactor during incident

### Topic-specific invariants / traps
- For **Debuggability review**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
- add narrow structured instrumentation where evidence is missing
- use stable correlation IDs/state transitions
- prefer minimal root-cause fix plus regression test

## Mode B — REVIEW
Review не является пересказом diff. Ищи конкретный failure mode и подтверждай его surrounding code.

- check whether logging/metrics can reconstruct failure path
- find swallowed errors and missing state/version dimensions
- verify privacy-safe breadcrumbs
...
- form hypotheses and falsify them one by one
- use symbolication/TSAN/Instruments/network metrics/store fixtures appropriate to failure class
- bisect regression when source history exists

## Mode D — MIGRATION
Применяй `00_META/MIGRATION_PROTOCOL.md`.

- preserve or improve observability across architecture/tool migration
- map old/new metric names during transition
...
- reproduction no longer fails under original conditions
- regression test or runtime evidence
- production metric/crash trend follow-up for high-impact fix
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-18-10 «Debuggability review» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: достаточны ли logs/metrics/ids/state transitions для расследования без reproducing everything.
