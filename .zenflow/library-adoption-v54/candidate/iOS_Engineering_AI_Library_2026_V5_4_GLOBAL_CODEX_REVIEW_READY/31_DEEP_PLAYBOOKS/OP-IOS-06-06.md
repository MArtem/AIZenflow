...
id: OP-IOS-06-06
type: deep-playbook
source_skill: IOS-06-06
title: Custom transitions — Operational Deep Playbook
section: 06_UIKIT
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-06-06 — Custom transitions

## Mission
transitioning delegate, interactive transitions, cancellation, accessibility Reduce Motion и rotation

Этот playbook — operational-версия `06_UIKIT/IOS-06-06_CUSTOM_TRANSITIONS.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Custom transitions».
...
- For **Custom transitions**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-06-06 «Custom transitions» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: transitioning delegate, interactive transitions, cancellation, accessibility Reduce Motion и rotation.
