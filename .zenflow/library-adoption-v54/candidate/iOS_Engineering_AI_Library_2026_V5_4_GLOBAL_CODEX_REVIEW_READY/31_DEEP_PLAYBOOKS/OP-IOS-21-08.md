...
id: OP-IOS-21-08
type: deep-playbook
source_skill: IOS-21-08
title: Migration workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-08 — Migration workflow

## Mission
inventory → compatibility matrix → seam strategy → dual-run/bridge → tests → rollout → cleanup

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-08_MIGRATION_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Migration workflow».
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-08 «Migration workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: inventory → compatibility matrix → seam strategy → dual-run/bridge → tests → rollout → cleanup.
