...
id: OP-IOS-22-01
type: deep-playbook
source_skill: IOS-22-01
title: AVFoundation capture session — Operational Deep Playbook
section: 25_MEDIA_GRAPHICS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-22-01 — AVFoundation capture session

## Mission
спроектировать camera/mic capture session, authorization, configuration, interruptions, orientation, device switching, buffers и teardown

Этот playbook — operational-версия `25_MEDIA_GRAPHICS/IOS-22-01_AVFOUNDATION_CAPTURE.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «AVFoundation capture session».
...
- Test interruption/device capability/resource pressure; avoid blocking realtime/render callbacks and bound buffers.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-22-01 «AVFoundation capture session» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: спроектировать camera/mic capture session, authorization, configuration, interruptions, orientation, device switching, buffers и teardown.
