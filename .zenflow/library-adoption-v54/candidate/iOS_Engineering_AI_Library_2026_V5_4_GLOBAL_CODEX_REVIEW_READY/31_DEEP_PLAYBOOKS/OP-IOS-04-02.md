...
id: OP-IOS-04-02
type: deep-playbook
source_skill: IOS-04-02
title: State management — Operational Deep Playbook
section: 04_ARCHITECTURE
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-04-02 — State management

## Mission
выбрать source of truth, state transitions, derived state и side effects без дублирования state

Этот playbook — operational-версия `04_ARCHITECTURE/IOS-04-02_STATE_MANAGEMENT.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «State management».
...
- For **State management**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-04-02 «State management» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: выбрать source of truth, state transitions, derived state и side effects без дублирования state.
