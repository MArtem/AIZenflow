...
id: OP-IOS-03-12
type: deep-playbook
source_skill: IOS-03-12
title: Priority/executors — Operational Deep Playbook
section: 03_CONCURRENCY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-03-12 — Priority/executors

## Mission
не злоупотреблять priority; проверять inheritance, UI responsiveness и custom executor necessity

Этот playbook — operational-версия `03_CONCURRENCY/IOS-03-12_PRIORITY.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Priority/executors».
...
- For **Priority/executors**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-03-12 «Priority/executors» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: не злоупотреблять priority; проверять inheritance, UI responsiveness и custom executor necessity.
