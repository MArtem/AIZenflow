...
id: OP-IOS-21-06
type: deep-playbook
source_skill: IOS-21-06
title: Performance optimization workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-06 — Performance optimization workflow

## Mission
define metric → measure baseline → find bottleneck → patch → compare → regression guard

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-06_PERFORMANCE_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Performance optimization workflow».
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-06 «Performance optimization workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: define metric → measure baseline → find bottleneck → patch → compare → regression guard.
