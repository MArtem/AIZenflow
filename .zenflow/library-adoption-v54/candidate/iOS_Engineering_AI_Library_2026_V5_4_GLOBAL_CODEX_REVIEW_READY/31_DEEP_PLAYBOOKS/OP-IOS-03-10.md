...
id: OP-IOS-03-10
type: deep-playbook
source_skill: IOS-03-10
title: Actor reentrancy — Operational Deep Playbook
section: 03_CONCURRENCY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-03-10 — Actor reentrancy

## Mission
проверить инварианты до/после await и защитить state transitions от interleaving

Этот playbook — operational-версия `03_CONCURRENCY/IOS-03-10_REENTRANCY.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Actor reentrancy».
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-03-10 «Actor reentrancy» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: проверить инварианты до/после await и защитить state transitions от interleaving.
