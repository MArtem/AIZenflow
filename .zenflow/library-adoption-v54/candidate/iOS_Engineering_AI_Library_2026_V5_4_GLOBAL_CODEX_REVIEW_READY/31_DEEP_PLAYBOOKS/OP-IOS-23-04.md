...
id: OP-IOS-23-04
type: deep-playbook
source_skill: IOS-23-04
title: ML model update/distribution — Operational Deep Playbook
section: 26_ML_VISION_AR
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-23-04 — ML model update/distribution

## Mission
версионировать models, validate signatures/compatibility, staged rollout, fallback и storage budget

Этот playbook — operational-версия `26_ML_VISION_AR/IOS-23-04_MODEL_UPDATE.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «ML model update/distribution».
...
- For **ML model update/distribution**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-23-04 «ML model update/distribution» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: версионировать models, validate signatures/compatibility, staged rollout, fallback и storage budget.
