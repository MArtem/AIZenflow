...
id: OP-IOS-21-05
type: deep-playbook
source_skill: IOS-21-05
title: Concurrency remediation workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-05 — Concurrency remediation workflow

## Mission
diagnostics inventory → classify isolation/Sendable/lifetime → fix roots → stress tests

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-05_CONCURRENCY_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Concurrency remediation workflow».
...
- Inspect closure captures/reference fields/generic constraints; prefer making data immutable/value-semantic over unchecked conformance.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-05 «Concurrency remediation workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: diagnostics inventory → classify isolation/Sendable/lifetime → fix roots → stress tests.
