...
id: OP-IOS-16-05
type: deep-playbook
source_skill: IOS-16-05
title: Code generation — Operational Deep Playbook
section: 16_BUILD_MODULARITY_TOOLING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-16-05 — Code generation

## Mission
детерминированный generation, checked-in policy, schema versioning, CI verification и no hidden side effects

Этот playbook — operational-версия `16_BUILD_MODULARITY_TOOLING/IOS-16-05_CODEGEN.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- target/module graph and build timings
- SPM package graph/version pins/plugins/macros
- generated-code/configuration source of truth
- Xcode build settings and CI parity
- binary size/linking/resource ownership
- Конкретные call sites/owners/tests, затронутые именно задачей «Code generation».
- Deployment/availability/toolchain ограничения до предложения нового API.

## Invariants
- module boundaries reflect ownership/change boundaries
- build tooling is reproducible in CI
- generated files are changed at generator source
- dependency versions are deterministic
- tooling does not expose secrets or silently modify developer machines

### Topic-specific invariants / traps
- For **Code generation**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
- reduce graph complexity before creating more modules
- cache only deterministic artifacts
- keep plugins/macros narrow and tested
- make configuration explicit per build environment

## Mode B — REVIEW
Review не является пересказом diff. Ищи конкретный failure mode и подтверждай его surrounding code.

- find cycles, duplicate dependencies/resources and unstable version rules
- check script phases input/output declarations and secret handling
- measure macro/plugin/build impact
...
- use build logs/timing summary/dependency graph before restructuring
- separate compiler, linker, codegen and test bottlenecks

## Mode D — MIGRATION
Применяй `00_META/MIGRATION_PROTOCOL.md`.

- move modules/packages incrementally with API compatibility
- keep old build path until CI proves parity for risky generator/tooling changes
...
- clean CI-like build
- package resolution reproducibility
- incremental build timing comparison when optimization is goal
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-16-05 «Code generation» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: детерминированный generation, checked-in policy, schema versioning, CI verification и no hidden side effects.
