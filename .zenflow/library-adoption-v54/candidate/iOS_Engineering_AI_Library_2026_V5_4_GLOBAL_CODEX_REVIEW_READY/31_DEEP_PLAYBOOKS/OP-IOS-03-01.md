...
id: OP-IOS-03-01
type: deep-playbook
source_skill: IOS-03-01
title: Миграция на Swift 6 strict concurrency — Operational Deep Playbook
section: 03_CONCURRENCY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-03-01 — Миграция на Swift 6 strict concurrency

## Mission
исправлять data-race design через isolation/ownership вместо подавления diagnostics

Этот playbook — operational-версия `03_CONCURRENCY/IOS-03-01_STRICT_CONCURRENCY_MIGRATION.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Миграция на Swift 6 strict concurrency».
...
- For **Миграция на Swift 6 strict concurrency**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-03-01 «Миграция на Swift 6 strict concurrency» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: исправлять data-race design через isolation/ownership вместо подавления diagnostics.
