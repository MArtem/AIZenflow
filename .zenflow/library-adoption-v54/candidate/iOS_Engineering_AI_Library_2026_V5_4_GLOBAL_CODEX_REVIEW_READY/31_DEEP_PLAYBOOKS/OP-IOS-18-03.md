...
id: OP-IOS-18-03
type: deep-playbook
source_skill: IOS-18-03
title: Crash triage — Operational Deep Playbook
section: 18_OBSERVABILITY_DEBUGGING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-18-03 — Crash triage

## Mission
symbolication, affected versions/devices, stack ownership, reproduction, regression range и fix verification

Этот playbook — operational-версия `18_OBSERVABILITY_DEBUGGING/IOS-18-03_CRASH_TRIAGE.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Crash triage».
...
- For **Crash triage**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-18-03 «Crash triage» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: symbolication, affected versions/devices, stack ownership, reproduction, regression range и fix verification.
