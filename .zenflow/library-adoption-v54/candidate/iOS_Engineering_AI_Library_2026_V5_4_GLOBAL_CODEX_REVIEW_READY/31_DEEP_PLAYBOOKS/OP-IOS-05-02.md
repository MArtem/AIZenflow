...
id: OP-IOS-05-02
type: deep-playbook
source_skill: IOS-05-02
title: State ownership — Operational Deep Playbook
section: 05_SWIFTUI
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-05-02 — State ownership

## Mission
различать owned state, bindings, environment dependencies и derived state

Этот playbook — operational-версия `05_SWIFTUI/IOS-05-02_STATE_OWNERSHIP.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «State ownership».
...
- For **State ownership**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-05-02 «State ownership» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: различать owned state, bindings, environment dependencies и derived state.
