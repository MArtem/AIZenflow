...
id: OP-IOS-05-05
type: deep-playbook
source_skill: IOS-05-05
title: Async UI state — Operational Deep Playbook
section: 05_SWIFTUI
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-05-05 — Async UI state

## Mission
loading/content/empty/error/refresh/cancel states без races и stale updates

Этот playbook — operational-версия `05_SWIFTUI/IOS-05-05_ASYNC_UI.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Async UI state».
...
- For **Async UI state**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-05-05 «Async UI state» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: loading/content/empty/error/refresh/cancel states без races и stale updates.
