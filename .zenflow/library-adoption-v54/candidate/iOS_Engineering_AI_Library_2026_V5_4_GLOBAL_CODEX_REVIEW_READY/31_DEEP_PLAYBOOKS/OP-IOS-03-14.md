...
id: OP-IOS-03-14
type: deep-playbook
source_skill: IOS-03-14
title: Concurrency code review — Operational Deep Playbook
section: 03_CONCURRENCY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-03-14 — Concurrency code review

## Mission
искать races, unchecked sendability, actor leaks, cancellation bugs, continuation misuse и hidden main-thread work

Этот playbook — operational-версия `03_CONCURRENCY/IOS-03-14_CONCURRENCY_CODE_REVIEW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Concurrency code review».
...
- Prove exactly-one resume on success/error/cancellation and handle callback APIs that may synchronously invoke completion.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-03-14 «Concurrency code review» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: искать races, unchecked sendability, actor leaks, cancellation bugs, continuation misuse и hidden main-thread work.
