...
id: OP-IOS-03-05
type: deep-playbook
source_skill: IOS-03-05
title: Structured concurrency — Operational Deep Playbook
section: 03_CONCURRENCY
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-03-05 — Structured concurrency

## Mission
использовать async let/task groups с cancellation/error propagation и понятным lifetime

Этот playbook — operational-версия `03_CONCURRENCY/IOS-03-05_STRUCTURED_CONCURRENCY.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Structured concurrency».
...
- For **Structured concurrency**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-03-05 «Structured concurrency» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: использовать async let/task groups с cancellation/error propagation и понятным lifetime.
