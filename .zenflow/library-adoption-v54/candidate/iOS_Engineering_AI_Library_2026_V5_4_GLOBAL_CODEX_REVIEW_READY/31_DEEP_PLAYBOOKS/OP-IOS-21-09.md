...
id: OP-IOS-21-09
type: deep-playbook
source_skill: IOS-21-09
title: Production incident workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-09 — Production incident workflow

## Mission
stabilize → evidence → scope → minimal mitigation → verification → permanent fix/postmortem

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-09_INCIDENT_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Production incident workflow».
...
- For **Production incident workflow**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-09 «Production incident workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: stabilize → evidence → scope → minimal mitigation → verification → permanent fix/postmortem.
