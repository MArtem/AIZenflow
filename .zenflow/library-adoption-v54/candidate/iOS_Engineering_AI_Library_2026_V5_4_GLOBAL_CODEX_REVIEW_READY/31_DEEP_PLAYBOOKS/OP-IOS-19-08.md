...
id: OP-IOS-19-08
type: deep-playbook
source_skill: IOS-19-08
title: Callback/Combine→async refactor — Operational Deep Playbook
section: 19_CODE_REVIEW_REFACTOR
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-19-08 — Callback/Combine→async refactor

## Mission
сохранить cancellation/backpressure/threading semantics и incremental interoperability

Этот playbook — operational-версия `19_CODE_REVIEW_REFACTOR/IOS-19-08_ASYNC_REFACTOR.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Callback/Combine→async refactor».
...
- For **Callback/Combine→async refactor**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-19-08 «Callback/Combine→async refactor» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: сохранить cancellation/backpressure/threading semantics и incremental interoperability.
