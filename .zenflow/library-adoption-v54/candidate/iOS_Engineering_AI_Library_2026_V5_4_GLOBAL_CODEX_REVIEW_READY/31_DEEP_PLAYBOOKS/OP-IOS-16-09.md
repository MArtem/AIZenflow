...
id: OP-IOS-16-09
type: deep-playbook
source_skill: IOS-16-09
title: Configuration/secrets — Operational Deep Playbook
section: 16_BUILD_MODULARITY_TOOLING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-16-09 — Configuration/secrets

## Mission
разделить public config и secrets, xcconfig/env/CI handling, no secret in bundle/source

Этот playbook — operational-версия `16_BUILD_MODULARITY_TOOLING/IOS-16-09_CONFIG_SECRETS.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Configuration/secrets».
...
- For **Configuration/secrets**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-16-09 «Configuration/secrets» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: разделить public config и secrets, xcconfig/env/CI handling, no secret in bundle/source.
