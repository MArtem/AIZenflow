...
id: OP-IOS-18-05
type: deep-playbook
source_skill: IOS-18-05
title: Bug reproduction — Operational Deep Playbook
section: 18_OBSERVABILITY_DEBUGGING
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-18-05 — Bug reproduction

## Mission
reduce nondeterminism, capture environment/state, seed/clock/network fixtures и minimal repro

Этот playbook — operational-версия `18_OBSERVABILITY_DEBUGGING/IOS-18-05_BUG_REPRO.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Bug reproduction».
...
- For **Bug reproduction**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-18-05 «Bug reproduction» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: reduce nondeterminism, capture environment/state, seed/clock/network fixtures и minimal repro.
