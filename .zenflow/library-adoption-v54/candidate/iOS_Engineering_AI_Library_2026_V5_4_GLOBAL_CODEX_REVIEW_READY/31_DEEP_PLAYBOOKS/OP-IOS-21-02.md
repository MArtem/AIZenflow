...
id: OP-IOS-21-02
type: deep-playbook
source_skill: IOS-21-02
title: Bugfix workflow — Operational Deep Playbook
section: 21_AGENT_WORKFLOWS
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-21-02 — Bugfix workflow

## Mission
reproduce → hypothesis → evidence → minimal fix → regression test → risk scan

Этот playbook — operational-версия `21_AGENT_WORKFLOWS/IOS-21-02_BUGFIX_WORKFLOW.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Bugfix workflow».
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-21-02 «Bugfix workflow» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: reproduce → hypothesis → evidence → minimal fix → regression test → risk scan.
