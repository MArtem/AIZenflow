...
id: OP-IOS-25-08
type: deep-playbook
source_skill: IOS-25-08
title: Crash reporting SDK — Operational Deep Playbook
section: 28_ECOSYSTEM_INTEROP
baseline: project-aware / Swift 6 safety target / 2026
---

# OP-IOS-25-08 — Crash reporting SDK

## Mission
symbolication, user identifiers policy, breadcrumbs redaction, release/version mapping and startup cost

Этот playbook — operational-версия `28_ECOSYSTEM_INTEROP/IOS-25-08_CRASH_SDK.md`. Используй его, когда задача требует реального изменения/ревью/диагностики/миграции, а не только справочного ответа.
...
- Конкретные call sites/owners/tests, затронутые именно задачей «Crash reporting SDK».
...
- For **Crash reporting SDK**, derive at least one concrete repository/task failure scenario from the source skill, actual owners/callers/tests and acceptance criteria; record the invariant that would falsify the proposed change.
...
```text
Работай как Staff/Principal iOS engineer. Применяй OP-IOS-25-08 «Crash reporting SDK» и V2_EXECUTION_PROTOCOL.
...
Основной фокус: symbolication, user identifiers policy, breadcrumbs redaction, release/version mapping and startup cost.
