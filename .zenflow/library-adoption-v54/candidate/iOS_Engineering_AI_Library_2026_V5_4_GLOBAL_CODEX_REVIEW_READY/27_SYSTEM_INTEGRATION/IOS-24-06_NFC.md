...
5. Проверь: entitlement, session timeout, tag state, errors, privacy, device capability.
...
## Готовый prompt
```text
Примени skill IOS-24-06: «Core NFC» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: session lifetime, entitlements, polling/read/write flows, user prompts, errors and device support.
Проверь: entitlement, session timeout, tag state, errors, privacy, device capability.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-24-06.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
