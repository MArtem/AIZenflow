...
## Назначение
Template constraints, scene lifecycle, entitlements, driver distraction rules and shared navigation state.
...
5. Проверь: entitlement, scene state, template limits, safe interaction, navigation sync, fallback.
...
## Готовый prompt
```text
Примени skill IOS-24-13: «CarPlay» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: template constraints, scene lifecycle, entitlements, driver distraction rules and shared navigation state.
Проверь: entitlement, scene state, template limits, safe interaction, navigation sync, fallback.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-24-13.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
