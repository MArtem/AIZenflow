...
5. Проверь: capability, token trust, session interruption, precision, privacy, fallback.
...
## Готовый prompt
```text
Примени skill IOS-24-09: «Nearby Interaction» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: capability/session lifecycle, discovery token exchange, precision availability and privacy.
Проверь: capability, token trust, session interruption, precision, privacy, fallback.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-24-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
