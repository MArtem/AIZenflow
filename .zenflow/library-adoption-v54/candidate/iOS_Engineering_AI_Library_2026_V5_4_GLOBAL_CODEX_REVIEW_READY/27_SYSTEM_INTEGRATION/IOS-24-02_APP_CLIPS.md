...
5. Проверь: size budget, invocation, auth/data migration, privacy, fallback, analytics.
...
## Готовый prompt
```text
Примени skill IOS-24-02: «App Clips» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: малый binary, invocation URLs, ephemeral authorization, handoff to full app и data boundaries.
Проверь: size budget, invocation, auth/data migration, privacy, fallback, analytics.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-24-02.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
