...
5. Проверь: discovery privacy, session state, ordering, large transfers, background, auth.
...
## Готовый prompt
```text
Примени skill IOS-24-08: «MultipeerConnectivity» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: peer discovery, session state, security/encryption assumptions, transfer backpressure and lifecycle.
Проверь: discovery privacy, session state, ordering, large transfers, background, auth.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-24-08.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
