...
2. Зафиксируй инварианты и failure modes именно для задачи: **tests/observability first, isolate side effects, reduce crash/race risk, then refactor**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-20-08: «Legacy module stabilization».
...
Особый фокус: tests/observability first, isolate side effects, reduce crash/race risk, then refactor.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-20-08.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
