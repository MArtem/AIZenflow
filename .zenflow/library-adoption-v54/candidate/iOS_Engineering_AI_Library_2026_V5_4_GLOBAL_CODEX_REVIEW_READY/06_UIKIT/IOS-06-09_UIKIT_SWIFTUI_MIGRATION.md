...
id: IOS-06-09
type: skill
title: Постепенная миграция UIKit→SwiftUI
domain: UIKit
priority: production
baseline: Swift 6 / modern iOS
---

# Постепенная миграция UIKit→SwiftUI
...
2. Зафиксируй инварианты и failure modes именно для задачи: **выбрать seams, hosting boundaries, shared models и rollback path без big-bang rewrite**.
...
5. Проверь тематические quality gates: lifecycle, containment, main-thread, layout/reuse, ARC, accessibility.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-06-09: «Постепенная миграция UIKit→SwiftUI».
...
Особый фокус: выбрать seams, hosting boundaries, shared models и rollback path без big-bang rewrite.
Проверь: lifecycle, containment, main-thread, layout/reuse, ARC, accessibility.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-06-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
