...
2. Зафиксируй инварианты и failure modes именно для задачи: **проверить инварианты до/после await и защитить state transitions от interleaving**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-03-10: «Actor reentrancy».
...
Особый фокус: проверить инварианты до/после await и защитить state transitions от interleaving.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-03-10.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
