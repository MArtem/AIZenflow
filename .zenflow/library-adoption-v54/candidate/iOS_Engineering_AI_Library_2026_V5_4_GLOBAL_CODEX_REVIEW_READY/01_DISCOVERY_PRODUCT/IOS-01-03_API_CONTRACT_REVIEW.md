...
id: IOS-01-03
type: skill
title: Разбор backend/API контракта
domain: Discovery & Product
priority: production
baseline: Swift 6 / modern iOS
---

# Разбор backend/API контракта
...
2. Зафиксируй инварианты и failure modes именно для задачи: **проверить schema, optionality, enums, versioning, pagination, errors, idempotency, timestamps/timezones, backward compatibility**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-01-03: «Разбор backend/API контракта».
...
Особый фокус: проверить schema, optionality, enums, versioning, pagination, errors, idempotency, timestamps/timezones, backward compatibility.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-01-03.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
