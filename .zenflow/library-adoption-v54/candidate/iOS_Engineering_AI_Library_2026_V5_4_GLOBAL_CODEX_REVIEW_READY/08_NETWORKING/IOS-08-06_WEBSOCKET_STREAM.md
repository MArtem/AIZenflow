...
2. Зафиксируй инварианты и failure modes именно для задачи: **connection state machine, heartbeat, reconnect, backoff, ordering, deduplication и lifecycle**.
...
## Готовый prompt
```text
Ты работаешь как Staff iOS engineer. Примени skill IOS-08-06: «WebSocket/streaming».
...
Особый фокус: connection state machine, heartbeat, reconnect, backoff, ordering, deduplication и lifecycle.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-08-06.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
