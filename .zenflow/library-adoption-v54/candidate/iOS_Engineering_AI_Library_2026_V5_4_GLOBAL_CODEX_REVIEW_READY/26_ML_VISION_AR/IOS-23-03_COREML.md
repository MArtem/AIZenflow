...
5. Проверь: model version, compute units, preprocessing parity, latency, memory, thermal.
...
## Готовый prompt
```text
Примени skill IOS-23-03: «Core ML inference» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: model loading, compute units, input preprocessing, prediction scheduling, memory/thermal и versioning.
Проверь: model version, compute units, preprocessing parity, latency, memory, thermal.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-23-03.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
