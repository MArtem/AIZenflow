...
5. Проверь: data minimization, consent, retention, on-device, logging, deletion.
...
## Готовый prompt
```text
Примени skill IOS-23-09: «ML data privacy» как Staff iOS engineer.
...
Сначала опиши lifecycle/state machine и системные ограничения. Особый фокус: минимизировать captured media/features, prefer on-device, redact/store safely и define retention.
Проверь: data minimization, consent, retention, on-device, logging, deletion.
...
Глубокая operational-версия этого skill: `31_DEEP_PLAYBOOKS/OP-IOS-23-09.md`. Она задаёт обязательные факты, инварианты, режимы исполнения, failure modes и verification ladder.
