...
if [[ $# -ne 2 ]]; then
  echo "Usage: $0 <PackageName> <OutputDirectory>"
  exit 1
fi
...
if [[ ! "$PACKAGE_NAME" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then
  echo "❌ Name must be a valid Swift identifier: $PACKAGE_NAME"
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
TEMPLATE_DIR="$ROOT_DIR/Templates/PackageTemplate"
...
python3 - "$DEST_DIR" "$PACKAGE_NAME" <<'PY'
from pathlib import Path
import os, sys
root = Path(sys.argv[1])
name = sys.argv[2]
placeholder = "{{PackageName}}"
# Rename directories first, top-down.
for path in sorted([p for p in root.rglob('*') if p.is_dir()], key=lambda p: len(p.parts)):
    if placeholder in path.name:
        new = path.with_name(path.name.replace(placeholder, name))
        path.rename(new)
# Rename files.
