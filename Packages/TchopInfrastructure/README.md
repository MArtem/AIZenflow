...
- **No hidden umbrella coupling**: umbrella targets may re-export only directly related targets. Database packages must not implicitly export navigation, sync, analytics, or UI packages.

## Current target map

- **Core/data**: `SyncCore`, `TchopDatabaseCore`, `TchopCoreDataDatabase`, `TchopSwiftDataDatabase`, `TchopDatabaseComposition`, `TchopDatabase`.
- **Networking/runtime**: `TchopNetworking`, `TchopNetworkTesting`, `TchopCache`, `TchopErrors`, `TchopAnalytics`.
