...
## Goal
Keep `source-app` implementation and documentation aligned with the current product contract while minimizing unnecessary complexity, context churn, and verification cost.
...
- Scope: `./source-app`, `./source-appShareExtension`, `./source-appWidgetExtension`, `./Packages`, `./docs`, `./.codex/skills/ios-content-cards`.
- Explicitly exclude `./app test targets`.
...
- Candidate files: `./source-app/App/AppState.swift`, `./source-app/Services/UserSessionService.swift`, `./source-app/Models/AccountProfileSummary.swift`, `./source-app/Views/AppRootView.swift`.
...
- Candidate files: `./source-app/Persistence/AppDatabase.swift`, `./source-app/Persistence/AppContentRecord.swift`, `./source-app/Repositories/AppContentRepository.swift`, `./source-app/ViewModels/AppShellViewModel.swift`.
- Also inspect package database contracts under `./Packages/source-app infrastructure` where app/database boundaries are involved.
- Required output per finding: severity P0-P3, affected files, evidence, why it is a problem, target state, remediation order, verification need.
...
- Candidate files: `./source-app/Views/AppRootView.swift`, `./source-app/Navigation/DeepLinkManager.swift`, `./source-app/Views/Tabs/NewsTabRootView.swift`.
...
- Candidate areas: `./Packages/source-app infrastructure/Sources/source-appShareSupport`, `./Packages/source-app infrastructure/Sources/source-appCache`, `./Packages/source-app infrastructure/Sources/source-appNetworking`.
...
- Inspect all SwiftUI files outside `./app test targets` for hot-path work: sync file/image/media work in `body`, heavy repeated shadows/blur/masks, unstable identity, broad observable dependencies, `AnyView`, computed maps/sorts/filters in render paths, side effects in `body`, and unnecessary layout invalidation.
- Scope includes `./source-app/**/*.swift`, `./source-appShareExtension/**/*.swift`, `./source-appWidgetExtension/**/*.swift`, and package SwiftUI views if present.
...
- Do not touch `./app test targets` unless the user explicitly asks.
...
- Completed now: whole-project static review pass across app/packages/extensions using updated docs/prompts; applied one minimal compile-safety fix in `./source-appShareExtension/ShareViewController.swift` to align share-extension composer publish callback with the current `FeedComposerViewModel` one-argument `publishAction` signature.
...
- Completed now: consistency cleanup batch from review findings:
  - made `./source-app/ViewModels/AppShellViewModel.swift` require an explicit `FeedCardPersisting` repository for `FeedCardStore`, preventing accidental memory-only production feed persistence.
  - removed the matching default in `./source-app/ViewModels/NewsFeedViewModel.swift` so feed runtime receives the same explicit store from app composition.
  - changed `./Packages/source-app infrastructure/Sources/source-appNetworking/source-appNetworking.swift` logging interceptor default logger from `print` to no-op.
  - localized visible share/widget extension strings through shared localization resources in `./Packages/source-app infrastructure`.
...
- Completed now: local-feed runtime cleanup pass:
  - removed `PhotoCardModel`/`TextCardModel` callback plumbing from `./source-app/Views/News/NewsFeedView.swift` and `./source-app/Views/Tabs/NewsTabRootView.swift`; feed taps/actions now go through local published cards only.
  - removed unused remote-card translation/action entry points from `./source-app/ViewModels/NewsFeedViewModel.swift`.
  - stopped app bootstrap from resolving fixture fallback feed content in `./source-app/App/AppDIContainer.swift`.
  - stopped seeding JSON-backed feed snapshots in `./source-app/Persistence/AppDataSeeder.swift`; seeding now only creates default channels.
  - changed `./source-app/Services/FeedAPIManager.swift` stub feed fetches to return an empty payload until a real backend contract exists.
  - removed bundled `StubFeedResponse*.json` resources from `./source-app/Resources` and from `./source-app.xcodeproj/project.pbxproj`.
- Verification: `git diff --check` only; no build/test/simulator run per current instruction.

- Completed now: removed the now-unused remote feed loading/action dependency from `./source-app/ViewModels/NewsFeedViewModel.swift` and app composition:
  ...
  - `./source-app/App/AppDIContainer.swift` creates the feed VM directly from channel/widget/error/local-card dependencies.
  - `./source-app/Repositories/AppContentRepository.swift` no longer exposes a `NewsFeedRepository` protocol surface to app composition; channel resolution remains the active repository contract.
- Verification: `git diff --check` only; no build/test/simulator run per current instruction.


- Completed now: executed requested steps 1, 2, and 4 after build-backed cleanup:
  - step 1 legacy localization/resource cleanup: removed unused `news.fallback.*`, `news.photo.pending.*`, and `news.text.pending.*` localization keys from active English/Russian resources; added active `news.feed.sourceFallback` in English/Russian/German and switched feed card source fallback to that key in `./source-app/Models/NewsFeedModels.swift`.
...
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded; no tests or simulator UI run.
...
- Active now: full read-only production audit requested and confirmed by user:
  - scope: entire project except `./app test targets`.
...
- Approved now: prepare Phase 1 + Phase 2 feed-card remediation implementation plan before coding.
  ...
  - Implementation plan, pending user confirmation before code changes:
    1. Domain/model reset in `./source-app/Models/NewsFeedModels.swift`:
    ...
    2. Persistence/store reset:
       - replace `FeedCardRecord` with source-neutral `FeedCardRecord` in `./source-app/Persistence/AppContentRecord.swift` and `./source-app/Persistence/AppDatabase.swift`; app is not released, so legacy migration compatibility does not block the rename/remodel.
       - replace `FeedCardRepository` / `FeedCardPersisting` with source-neutral `FeedCardRepository` / concrete persistence boundary in `./source-app/Repositories/AppContentRepository.swift`.
       - move feed-card store/persistence protocol out of `./source-app/ViewModels/AppShellViewModel.swift` so view-model files do not own repository contracts.
    ...
    3. App composition/share/widget naming reset:
       - replace `feedCardStore` and related DI naming in `./source-app/App/AppDIContainer.swift`, `./source-app/ViewModels/AppShellViewModel.swift`, and `./source-app/ViewModels/NewsFeedViewModel.swift`.
       - replace `SharedFeedCardSyncManager` with source-neutral shared feed-card import/sync naming in `./source-app/Shared` and `./source-appShareExtension/ShareViewController.swift`.
    ...
    4. Feed UI renderer reset in `./source-app/Views/News/NewsFeedView.swift`:
    ...
    5. Feed state/performance reset in `./source-app/ViewModels/NewsFeedViewModel.swift`:
    ...
    6. Media preview pipeline for smooth scroll:
       ...
       - remove `UIImage(contentsOfFile:)`, `AVAssetImageGenerator.copyCGImage`, `PDFDocument(url:)`, and repeated `FileManager.fileExists` from row body/computed render paths in `./source-app/Views/News/NewsFeedView.swift`.
    ...
    9. Verification after implementation, only with user-approved command scope:
       ...
       - run `plutil -lint ./source-app.xcodeproj/project.pbxproj` if Xcode project changes.
...
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded after fixes; no tests or simulator UI run.


- Completed now: targeted persistence and media import I/O cleanup follow-up:
  - changed `./source-app/ViewModels/AppShellViewModel.swift` so single-card like/comment/display-mode updates persist one `FeedCard` and update the affected in-memory row instead of rebuilding the whole feed array.
  - added a single-card persistence API in `./source-app/Repositories/AppContentRepository.swift`; build constraints forced the SwiftData lookup to remain warning-free rather than using `#Predicate`, but the store/update call path is now single-card scoped.
  - moved share-extension imported file copying in `./Packages/source-app infrastructure/Sources/source-appShareSupport/ShareItemImporter.swift` into a detached utility task so large share files are not copied on the main actor.
  - moved composer media data writes and document-picker file copies in `./source-app/Views/Composer/SharedCardComposerView.swift` into detached utility work where the API allows it; kept the synchronous Transferable import copy for the `FileRepresentation` closure because that closure is synchronous.
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded; no tests/simulator UI/Instruments run.


- Completed now: share/app-group robustness and feed visual rendering cleanup:
  - added per-file safe JSON loading in `./Packages/source-app infrastructure/Sources/source-appShareSupport/source-appShareSupport.swift` so one corrupted pending share item no longer blocks all valid cards.
  ...
  - changed `./source-app/Shared/SharedFeedCardSyncManager.swift` to sync valid pending feed cards, remove only processed files, and quarantine malformed files.
  - reduced repeated feed-row offscreen rendering cost in `./source-app/Views/News/NewsFeedView.swift` by replacing the per-card shadow with a cheap rounded border overlay.
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded; no tests/simulator UI/Instruments run.
...
- Completed now: Full Read-Only Production Audit completed in one pass using the new production checklist and stop list:
  - Scope inspected: `./source-app`, `./source-appShareExtension`, `./Packages`, `./docs`, and `./.codex/skills/ios-content-cards`; `./app test targets` was not touched. `./source-appWidgetExtension` does not exist as a source directory in this worktree; widget package code under `./Packages/source-app infrastructure/Sources/source-appWidgets` was included.
...
- Verification: after the remediation pass `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded. A final build also succeeded after feature-tab naming cleanup. No tests/simulator UI/manual validation/Instruments trace recording were run.


- Completed now: simulator login unblock for Instruments/manual feed testing:
  - changed `./source-app/App/AppDIContainer.swift` so the `.developmentStub` runtime uses `InMemoryAuthTokenStore()` instead of Keychain storage, avoiding unsigned simulator secure-storage failures while keeping Keychain for external/staging/production environments.
...
- Completed now: simulator Time Profiler scroll capture attempt and analysis:
  ...
  - captured a valid auto-stopped all-process Time Profiler trace at `./.zenflow/tasks/new-task-be0b/traces/source-app-feed-scroll-repeat.trace`.
  - exported analysis to `./.zenflow/tasks/new-task-be0b/traces/source-app-feed-scroll-repeat.analysis.json`.
  - findings: no `potential-hangs` >250ms; simulator does not provide Animation Hitches/SwiftUI lanes; source-app samples are overwhelmingly main-thread/AttributeGraph/scroll graph work, with visible cost from `NewsFeedScrollObserver` KVO / `AppShellViewModel.setNewsFeedNearTop(_:)` and no sampled evidence of synchronous media decoding during this run.
...
- Completed now: Scroll Hot Path Remediation block 1-4:
  - changed `./source-app/Views/News/NewsFeedView.swift` so `NewsFeedScrollObserver` forwards only near-top threshold transitions instead of every `contentOffset` update, preserving the existing `+` button visibility threshold.
  - split `./source-app/Views/ShellContentView.swift` bottom chrome observation into `ShellBottomChromeHostView`, so `isNewsFeedNearTop` invalidates the bottom chrome host rather than the whole shell/feed subtree.
  - added lightweight feed signposts for `ScrollNearTopStateChange`, `FeedCardAppear`, `FeedMediaPreviewLoad`, and `FeedVisibleContentRefresh` in `./source-app/Views/News/NewsFeedView.swift` and `./source-app/ViewModels/NewsFeedViewModel.swift`.
  - cleaned unused app-group fallback error presentation assignments in `./source-app/App/AppDIContainer.swift` to remove build warnings from the current branch.
...
- Completed now: fixed `+` button hide/show regression from the scroll-hot-path cleanup:
  - replaced the prior UIKit/KVO threshold observer in `./source-app/Views/News/NewsFeedView.swift` with a SwiftUI top-offset sentinel scoped to the feed `ScrollView` coordinate space.
...
- Completed now: fixed floating `+` hide/show regression after scroll optimization:
  - moved the feed scroll-position sentinel in `./source-app/Views/News/NewsFeedView.swift` out of `LazyVStack` into the stable scroll content wrapper so lazy eviction cannot reset the top-offset preference to the near-top default.
...
- Completed now: hardened floating `+` visibility path after manual check still showed the button always visible:
  - `./source-app/Views/News/NewsFeedView.swift` now uses native `onScrollGeometryChange` on iOS 18+ for boolean near-top tracking, with the sentinel/preference path retained only as the iOS 17 fallback.
  - `./source-app/Views/ShellContentView.swift` now stores feed near-top visibility in local `@State` and passes the boolean directly into bottom chrome, avoiding an indirect nested-observation dependency for the floating button.
...
- Completed now: corrected feed lazy rendering structure for production scroll performance:
  - removed the opaque `NewsFeedContentSectionView` wrapper from `./source-app/Views/News/NewsFeedView.swift`.
...
- Completed now: final `./source-app/Views/News/NewsFeedView.swift` cleanup after external review:
...
- Completed now: enterprise-scale iOS production governance/docs/prompts/skills/static-gates hardening pass:
  ...
  - Removed a hard-coded ReqRes demo API-key fallback from `./scripts/api_method_trace`; the script now requires `APP_REQRES_API_KEY` from environment when used.
...
- Completed now: full-project inline code documentation pass requested by user:
  - Scope covered all Swift source/test targets under `./source-app`, `./source-appShareExtension`, `./Packages/source-app infrastructure`, and `./app test targets`; generated/build artifacts were excluded.
...
- Verification: `git diff --check` succeeded; `./scripts/verify.sh low` succeeded with `BUILD SUCCEEDED`; custom Swift documentation scan succeeded. `swift test` in `./Packages/source-app infrastructure` was also attempted and failed on a pre-existing package test compile issue (`AppGroupJSONItemDirectoryStore<TestItem>` has no `remove(id:)` member in `./Packages/source-app infrastructure/Tests/source-appShareSupportTests/source-appShareSupportTests.swift:23`) plus existing warnings; this was not fixed because the current task was documentation-only.


- Completed now: fixed the package test compile failure found by `swift test` in `./Packages/source-app infrastructure`:
  - added `remove(id:)` convenience API to `./Packages/source-app infrastructure/Sources/source-appShareSupport/source-appShareSupport.swift`, delegating to the existing batch removal contract so `source-appShareSupportTests` can express single-item removal without duplicating array setup.
  - restored the expected concrete error-mapper convenience surface in `./Packages/source-app infrastructure/Sources/source-appErrors/source-appErrors.swift` by adding an `APIError` overload and default `context` arguments where the existing tests call the mapper directly.
  - updated `./Packages/source-app infrastructure/Tests/source-appLocalizationTests/source-appLocalizationTests.swift` expectations to match the current localization resources for `login.title` (`Welcome back` / `С возвращением`).
- Verification: `git diff --check` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 52 XCTest tests and 20 Swift Testing tests; `./scripts/verify.sh low` succeeded with `BUILD SUCCEEDED`. Remaining non-blocking warnings are Swift concurrency warnings in networking/package-test code and should be handled in a dedicated Swift 6/concurrency cleanup pass.


- Completed now: Swift concurrency / Swift 6 warning cleanup for `./Packages/source-app infrastructure`:
  - constrained `TaskBox` in `./Packages/source-app infrastructure/Sources/source-appNetworking/source-appNetworking.swift` to `Response: Sendable`, matching the `APIRequest`/`Task` call sites that already require sendable response values.
  - made database operation closures in `./Packages/source-app infrastructure/Sources/source-appDatabaseCore/source-appDatabaseCore.swift` explicitly `@Sendable` and adjusted the SwiftData operation wrapper so the strict-concurrency boundary is expressed in the API contract.
  - removed deprecated `Locale.languageCode` fallbacks from `./Packages/source-app infrastructure/Sources/source-appLocalization/source-appLocalization.swift` and `./Packages/source-app infrastructure/Sources/source-appOnDeviceAI/source-appOnDeviceAI.swift`.
  - changed share text import in `./Packages/source-app infrastructure/Sources/source-appShareSupport/ShareItemImporter.swift` so non-sendable `NSSecureCoding` payloads are decoded inside the provider callback and only sendable `ShareImportedTextItem` values cross the async continuation boundary.
  - replaced the Core Data test merge-policy global with an explicit `NSMergePolicy` instance in `./Packages/source-app infrastructure/Tests/source-appDatabaseTests/source-appDatabaseTests.swift`.
  - replaced mutable static URL protocol test state in `./Packages/source-app infrastructure/Tests/source-appNetworkingTests/source-appNetworkingTests.swift` with a locked sendable state holder and locked counter helpers for sendable request-handler closures.
- Verification: clean `swift test` in `./Packages/source-app infrastructure` succeeded with no Swift compiler warnings in the captured warning grep; regular `swift test` succeeded with 52 XCTest tests and 20 Swift Testing tests; `git diff --check`, `python3 ./scripts/check_docs_index.py`, `python3 ./scripts/validate_ios_production_framework.py`, and `./scripts/verify.sh low` all succeeded. Xcode build still prints a non-code AppIntents metadata note/warning for the share extension because it has no AppIntents dependency.


- Completed now: removed the remaining Xcode AppIntents metadata warning from share-extension builds:
  - updated `./source-app.xcodeproj/project.pbxproj` so all `source-appShareExtension` build configurations pass `-Xfrontend -disable-autolink-framework -Xfrontend AppIntents` through `OTHER_SWIFT_FLAGS`.
...
- Verification: `plutil -lint ./source-app.xcodeproj/project.pbxproj`, `git diff --check`, and `./scripts/verify.sh low` succeeded; captured build log no longer contains `Metadata extraction skipped` or `warning:` entries.
...
- Active now: full app test coverage expansion phase opened by user on 2026-05-26:
  ...
  - temporary override: `./app test targets` is allowed to be modified until the user explicitly says to stop writing tests; after that command, the previous no-touch rule returns automatically.
...
- Completed now: package and app test coverage expansion pass:
  - expanded `./Packages/source-app infrastructure` test coverage across SyncCore, Apple authentication, navigation, branding, cache, share support, localization, and app-error mapping surfaces.
  - realigned `./app test targets` with the current source-neutral feed-card runtime and SwiftData-only active persistence policy; removed stale remote feed repository/DTO assumptions from test doubles and feed view-model tests.
  ...
  - updated `./source-appUITests/source-appUITests.swift` to match the current localized password-visibility accessibility label.
...
- Verification: `git diff --check` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests; full `xcodebuild ... test` for `./source-app.xcodeproj`/`source-app` on iPhone 17 Pro iOS 26.0 succeeded; `./scripts/verify.sh medium` succeeded, including package tests, app/UI tests, and final debug build.


- Completed now: continued app-specific test coverage expansion after reconnect:
  - added `./app test targets/AppShellViewModelTests.swift` and `./app test targets/ShareExtensionRuntimeContractTests.swift` to the `app test targets` Xcode target so existing test files are actually compiled and executed.
  - updated `./app test targets/ShareExtensionRuntimeContractTests.swift` from stale `Local*` names to the current source-neutral `FeedCard` / `SharedFeedCardSyncManager` runtime.
  ...
  - added `./app test targets/NewsFeedViewModelTests.swift` coverage for canonical search-field order: `text`, `headline`, `subheadline`, `source`.
  - added a test-only injection initializer to `./source-app/Shared/SharedFeedCardSyncManager.swift` so app-group sync can be tested without a real app-group entitlement.
  - fixed `./app test targets/AppShellViewModelTests.swift` setup now that `UIConfigurationSnapshot` requires an explicit shell configuration.
- Verification: `git diff --check` succeeded; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded; full `xcodebuild ... test` for `./source-app.xcodeproj`/`source-app` on iPhone 17 Pro iOS 26.0 succeeded with the newly enabled tests; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests.


- Completed now: continued app-specific test coverage expansion and full verification stabilization:
  - added app launch configuration tests in `./app test targets/AppStateTests.swift` for UI-test in-memory SwiftData configuration and ReqRes external-auth environment resolution.
  - added `SessionStore` transition coverage in `./app test targets/AppStateTests.swift` to verify current-user exposure across signed-out/authenticated transitions.
  - added `ProfileTabViewModel` coverage in `./app test targets/AppShellViewModelTests.swift` for current-user summary sync plus navigation-restore preference optimistic success and rollback failure behavior.
...
- Verification: `git diff --check` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded.


- Completed now: feed/card runtime and composer contract test expansion:
  - added `./app test targets/NewsFeedViewModelTests.swift` coverage for whitespace-only search, multi-token same-field matching, equal-priority stable search ordering, search dismissal restoring the selected-channel snapshot, duplicate feed-card sync no-op behavior, targeted single-card update persistence, and missing-card update no-op behavior.
  - added `./app test targets/NewsFeedViewModelTests.swift` persistence coverage for `FeedCardRepository.saveCard` inserting a missing record and `loadCards()` returning cards newest-first by `createdAt`.
  - extended `./app test targets/TestDoubles/TestAppContentRepository.swift` to record single-card saves separately from batch saves, allowing tests to verify targeted persistence rather than broad batch writes.
  - added `./app test targets/ShareExtensionRuntimeContractTests.swift` composer contract coverage for required text behavior without media, removable text when media carries the draft, media removal making text required again, source URL publishing only with visible source text, and photo metadata publishing only non-empty fields.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/NewsFeedViewModelTests -only-testing:app test targets/AppContentRepositoryTests test` succeeded; targeted `xcodebuild ... -only-testing:app test targets/ShareExtensionRuntimeContractTests test` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests; `plutil -lint ./source-app.xcodeproj/project.pbxproj` and `git diff --check` succeeded.


- Completed now: AppState/navigation/auth lifecycle test expansion:
  - added `./app test targets/AppStateTests.swift` coverage for unauthenticated deep-link queuing, latest-pending-link replacement, authenticated immediate deep-link routing, failed sign-in not replaying pending links, restore failure falling back to signed-out state, opt-in navigation snapshot persistence, clean snapshot restore without persistence feedback loop, and disabling navigation restore clearing persisted snapshot plus resetting navigation.
  - added `RecordingDeepLinkManager` and navigation save-call tracking in `./app test targets/TestDoubles/AppStateTestDoubles.swift` so routing and snapshot persistence side effects are asserted directly.
  - fixed a production runtime issue surfaced by the new restore-failure test: `./source-app/App/AppState.swift` no longer traps with `assertionFailure` on recoverable session-restore errors; it maps the error through the app error manager and safely falls back to signed-out state.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/AppStateTests test` first exposed the restore-failure trap, then succeeded after the runtime fix; `git diff --check` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded.


- Completed now: channel/login/session-manager test coverage expansion:
  - added `./app test targets/AppStateTests.swift` coverage for `ChannelsStore` ordering, persisted-selection priority, invalid selection fallback, unknown channel rejection, and reset semantics that clear user selection without destroying the available channel snapshot.
  - added `./app test targets/AppStateTests.swift` coverage for `UserChannelSettingsRepository` ReqRes user normalization and default channel settings for unknown users.
  - added `./app test targets/LoginViewModelTests.swift` coverage for password visibility, weak default-password blocking, ReqRes registration routing, default-mode registration no-op behavior, submission throttling, Apple authorization cancellation, and Apple authorization failure error mapping.
  - added `./app test targets/AppStateTests.swift` coverage for `UserSessionService` token-backed sign-in persistence, token rollback when local user resolution fails, orphan-token cleanup, expired-token refresh, missing-refresh-token cleanup, and sign-out revoke semantics.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/ChannelsStoreTests -only-testing:app test targets/UserChannelSettingsRepositoryTests test` succeeded; targeted `xcodebuild ... -only-testing:app test targets/LoginViewModelTests test` succeeded; targeted `xcodebuild ... -only-testing:app test targets/UserSessionServiceTests test` first exposed a missing test import and then succeeded after adding `source-appleAuthentication`; `git diff --check` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `swift test` in `./Packages/source-app infrastructure` succeeded with 59 XCTest tests and 37 Swift Testing tests; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded.


- Completed now: bridge and feed-card model contract test expansion:
  - added `./app test targets/AppStateTests.swift` coverage for widget feed sync save/empty/clear behavior, widget sync failure reporting, APNs token forwarding, registration failure forwarding, remote notification payload forwarding, and push-manager failure reporting.
  - fixed recoverable bridge failure behavior in `./source-app/App/AppWidgetBridge.swift` and `./source-app/App/AppPushNotificationBridge.swift`: widget/push bridge failures now go through `AppErrorManaging` without debug/test `assertionFailure` traps.
  - added `./app test targets/NewsFeedViewModelTests.swift` `NewsFeedModelContractTests` coverage for composer text ordering/trimming, legacy persisted `FeedCard` interaction defaults, translation payload trimming/source exclusion, nil channel scoping, detail-route fallback fields, and media display-title contracts.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/AppBridgeTests test` first exposed an expected localized-description assertion mismatch and then succeeded after fixing the test expectation; targeted `xcodebuild ... -only-testing:app test targets/NewsFeedModelContractTests test` first exposed a malformed string literal in the new test and then succeeded after fixing it; final `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, and `swift test` in `./Packages/source-app infrastructure` all succeeded.


- Completed now: app-specific shell/composer/action-coordinator test expansion:
  - added `./app test targets/AppShellViewModelTests.swift` coverage for menu toggle/close, composer presentation using the selected channel, channel selection refreshing feed scope and syncing share-extension session context, and composer publishing refreshing the feed plus dismissing the active composer.
  - added a test-only file-manager injection initializer to `./source-app/Shared/ShareExtensionSessionContextManager.swift` so app-group session context can be verified without real entitlements.
  - fixed recoverable UI-configuration refresh failure behavior in `./source-app/ViewModels/AppShellViewModel.swift`: refresh errors now go through `AppErrorManaging` without debug/test `assertionFailure` traps.
  - added `./app test targets/NewsFeedViewModelTests.swift` coverage for `NewsFeedCardActionCoordinator` queued additive actions, per-card clear semantics, and `cancelAll()` task cancellation/queue cleanup.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/AppShellViewModelTests test` first exposed a missing test file-manager double, then succeeded after adding a local double; targeted `xcodebuild ... -only-testing:app test targets/NewsFeedCardActionCoordinatorTests test` succeeded; final `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, and `swift test` in `./Packages/source-app infrastructure` all succeeded.


- Completed now: app-specific shell error and composer view-model contract test expansion:
  - added `./app test targets/AppShellViewModelTests.swift` coverage that failed UI-configuration refreshes are reported through `AppErrorManaging` while preserving the cached shell `showsFloatingActionButton` state.
  ...
  - added `./app test targets/ShareExtensionRuntimeContractTests.swift` `FeedComposerViewModelTests` coverage for empty publish no-op behavior, source-neutral text-card publishing, channel/title sourcing from `ChannelsStore`, and draft field visibility mutations through the view-model surface.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/AppShellViewModelTests test` succeeded; targeted `xcodebuild ... -only-testing:app test targets/FeedComposerViewModelTests test` succeeded; final `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, and `swift test` in `./Packages/source-app infrastructure` all succeeded.


- Completed now: launch-configuration and share-extension session-context test expansion:
  - added `./app test targets/AppStateTests.swift` coverage for default launch configuration, UI-test authenticated-session flags, invalid initial URL handling, and ReqRes external-auth default API-key behavior.
  - added `./app test targets/ShareExtensionRuntimeContractTests.swift` `ShareExtensionSessionContextManagerTests` coverage for authenticated session context round-trip through app-group JSON storage and signed-out context scrubbing of channel details.
- Verification: targeted `xcodebuild ... -only-testing:app test targets/AppLaunchConfigurationTests test` succeeded; targeted `xcodebuild ... -only-testing:app test targets/ShareExtensionSessionContextManagerTests test` succeeded; final `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, and `swift test` in `./Packages/source-app infrastructure` all succeeded.

- Completed now: P0 feed/composer/FAB UI regression coverage:
  - made `./source-app/App/AppDIContainer.swift` use deterministic UI-test UI configuration by avoiding persisted standard `UserDefaults` shell configuration in UI-test mode, so tests cannot inherit a hidden floating action button from previous runtime state.
  - added stable UI-test identifiers in `./source-app/Views/Composer/SharedCardComposerView.swift` for composer root, body input, and publish/cancel actions.
  - kept `./source-app/Views/Tabs/FloatingActionButton.swift` visual glass styling intact while preserving a stable accessibility identifier and adding XCTest label fallbacks for iOS 26 Liquid Glass exposure behavior.
  - hardened `./source-app/Views/News/NewsFeedView.swift` scroll/FAB proximity handling: empty feed keeps the plus button visible, iOS 18+ uses native scroll geometry, and older systems keep the preference-key fallback.
  - added `./source-appUITests/source-appUITests.swift` P0 UI tests for authenticated text-card publish into feed and feed scroll hide/restore behavior for the shell plus button.
- Verification: targeted UI tests for `testAuthenticatedUserCanPublishTextCardIntoFeed` and `testFeedScrollHidesAndRestoresFloatingActionButton` succeeded; `git diff --check` succeeded; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded; `(cd ./Packages/source-app infrastructure && swift test)` succeeded with 59 XCTest tests and 37 Swift Testing tests; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded.

- Completed now: feed MVVM contract hardening for scalable growth without speculative layers:
  - added `NewsFeedAction` in `./source-app/ViewModels/NewsFeedViewModel.swift` so feed UI/shell events enter the view model as product/user intents instead of direct mutation helper calls.
  - added `NewsFeedScreenState` in `./source-app/ViewModels/NewsFeedViewModel.swift` so `./source-app/Views/News/NewsFeedView.swift` reads one cheap screen-level presentation snapshot per SwiftUI body pass.
  ...
  - updated `./source-app/Views/News/NewsFeedView.swift`, `./source-app/Views/ShellContentView.swift`, `./source-app/ViewModels/AppShellViewModel.swift`, and `./source-app/App/AppState.swift` to use the action contract where feed events cross component boundaries.
  - updated `./app test targets/NewsFeedViewModelTests.swift` to exercise feed interactions/search/channel changes through the new action surface.
- Verification: first targeted `xcodebuild ... -only-testing:app test targets/NewsFeedViewModelTests test` exposed a Swift concurrency isolation issue in `NewsFeedViewModel.deinit`; removed the unsafe deinit access and reran successfully. Final verification succeeded with `git diff --check`, targeted `NewsFeedViewModelTests`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `(cd ./Packages/source-app infrastructure && swift test)`.



- Completed now: first-step feed card view-state boundary:
  - added `NewsFeedCardViewState`, `NewsFeedCardActionViewState`, and `NewsFeedCardTranslationViewState` in `./source-app/ViewModels/NewsFeedViewModel.swift`.
  - extended `NewsFeedScreenState` with `cardStates`, so `./source-app/Views/News/NewsFeedView.swift` renders rows from a prepared presentation snapshot instead of rebuilding translation/action/footer decisions in the view tree.
  - changed `NewsFeedCardRendererView` in `./source-app/Views/News/NewsFeedView.swift` to consume `NewsFeedCardViewState`; route, rendered card, footer action state, and translation presentation now come from the view-model snapshot.
  ...
  - added `./app test targets/NewsFeedViewModelTests.swift` assertions for the new screen/card-state action contract.
- Verification: `git diff --check` succeeded; targeted `xcodebuild ... -only-testing:app test targets/NewsFeedViewModelTests test` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded; `(cd ./Packages/source-app infrastructure && swift test)` succeeded.



- Completed now: feed screen-state presentation branching cleanup:
  - added `NewsFeedContentPresentation` in `./source-app/ViewModels/NewsFeedViewModel.swift` so empty/search-empty/cards branching is part of the screen-state snapshot instead of being recomputed in `./source-app/Views/News/NewsFeedView.swift`.
  ...
  - updated `./source-app/Views/News/NewsFeedView.swift` to keep `ForEach` directly inside the main `LazyVStack` while switching on `screenState.contentPresentation`.
  ...
  - extended `./app test targets/NewsFeedViewModelTests.swift` coverage for empty, no-results, card-list presentation, `hasVisibleCards`, and first-step card accessibility summary state.
- Verification: first targeted `xcodebuild ... -only-testing:app test targets/NewsFeedViewModelTests test` exposed a missing `return` in `NewsFeedViewModel.screenState`; fixed it and reran targeted tests successfully. Final verification succeeded with `git diff --check`, targeted `NewsFeedViewModelTests`, full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `(cd ./Packages/source-app infrastructure && swift test)`.



- Completed now: feed translation tap behavior presentation-state cleanup:
  - added `NewsFeedCardTranslationTapBehavior` in `./source-app/ViewModels/NewsFeedViewModel.swift` to encode restore-original, direct one-language translation, and language-picker behavior in card presentation state.
  - changed `NewsFeedCardTranslationViewState` to expose `tapBehavior` instead of leaking target-language decision logic into `./source-app/Views/News/NewsFeedView.swift`.
  ...
  - added deterministic `TestOnDeviceAIManager` support and translation tap-behavior assertions in `./app test targets/NewsFeedViewModelTests.swift`.
- Verification: `git diff --check` succeeded; targeted `xcodebuild ... -only-testing:app test targets/NewsFeedViewModelTests test` succeeded; full `xcodebuild -project ./source-app.xcodeproj -scheme source-app -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.0' CODE_SIGNING_ALLOWED=NO test` succeeded; `plutil -lint ./source-app.xcodeproj/project.pbxproj` succeeded; `(cd ./Packages/source-app infrastructure && swift test)` succeeded.
...
- Completed now: documentation split/export baseline for future projects:
  - created `./docs/documentation-split/app-specific/` with source-app-specific docs, task docs, feed/card contracts, source-app package skills, app-specific prompt/knowledge files, and manifest.
...
## Verification Status
- Latest verification succeeded with `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, full `xcodebuild ... test` for `./source-app.xcodeproj`/`source-app`, targeted `NewsFeedViewModelTests`, targeted P0 UI tests for composer publish and feed scroll/FAB behavior, and `swift test` in `./Packages/source-app infrastructure`.
...
Global reusable knowledge and source-app-specific knowledge are now split under `./docs/knowledge/`.
...
- Completed now: removed legacy remote/stub feed runtime surface from app/feed composition:
  - simplified `./source-app/Repositories/AppContentRepository.swift` to channel resolution plus feed card SwiftData persistence only; removed remote feed refresh/action sync helpers and network reachability dependency.
...
- Verification: `git diff --check` and `plutil -lint ./source-app.xcodeproj/project.pbxproj` only; no build/test/simulator run per current instruction.
...
- Completed now: build-backed follow-up cleanup across the three requested checks:
  ...
  - persistence leftovers review kept `FeedCardRecord` in the SwiftData schema for migration/backward-compatibility safety and clarified comments in `./source-app/Persistence/AppContentRecord.swift`; active runtime remains `FeedCardRecord`.
  - removed legacy UI-only remote card views/models from `./source-app/Views/News/PhotoCardView.swift`, `./source-app/Views/News/TextCardView.swift`, `./source-app/Views/News/PhotoActionView.swift`, `./source-app/Models/NewsFeedModels.swift`, and related preview samples/project references.
  - fixed share-extension publish/concurrency warnings by aligning `./source-appShareExtension/ShareViewController.swift` with boolean `publish()` and marking `NSItemProviderShareItemImporter` main-actor isolated in `./Packages/source-app infrastructure/Sources/source-appShareSupport/ShareItemImporter.swift`.
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and final `./scripts/verify.sh low` all succeeded; no tests/simulator UI run.
...
- Completed now: static end-to-end review of local-created feed card flow:
  ...
  - applied one contract fix in `./source-app/Models/NewsFeedModels.swift`: composer visible text fields now use canonical order `text`, `headline`, `subheadline`, `source`, matching published feed/feed card ordering.
- Verification: `git diff --check`, `plutil -lint ./source-app.xcodeproj/project.pbxproj`, and `./scripts/verify.sh low` succeeded; no tests/simulator UI run.
