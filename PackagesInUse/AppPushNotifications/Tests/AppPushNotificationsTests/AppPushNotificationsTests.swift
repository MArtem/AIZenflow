...
struct AppPushNotificationsTests {
    ...
    func defaultParserExtractsAlertAndCustomData() {
        ...
        #expect(payload.customData["route"] == "app://news/text/parrots")
    ...
    func managerPersistsTokenAndOpenedPayload() async throws {
        let suiteName = "AppPushNotificationsTests.\(UUID().uuidString)"
        ...
        #expect(state.lastOpenedPayload?.customData["route"] == "app://news")
    }
    ...
    func userDefaultsStoreSupportsConcurrentAccess() async throws {
        let suiteName = "AppPushNotificationsTests.Concurrent.\(UUID().uuidString)"
        let userDefaults = try #require(UserDefaults(suiteName: suiteName))
        defer {
            userDefaults.removePersistentDomain(forName: suiteName)
        }
