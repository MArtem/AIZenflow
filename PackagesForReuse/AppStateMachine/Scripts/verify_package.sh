...
[[ "${PACKAGE_NAME}" == "AppStateMachine" ]] || fail "package folder must be AppStateMachine"
...
STRICT_LOG="${SCRATCH_ROOT}/strict.log"
NORMAL_LOG="${SCRATCH_ROOT}/normal.log"

swift test --package-path "${PACKAGE_DIR}" --scratch-path "${SCRATCH_ROOT}/build-strict" -Xswiftc -strict-concurrency=complete 2>&1 | tee "${STRICT_LOG}"
swift test --package-path "${PACKAGE_DIR}" --scratch-path "${SCRATCH_ROOT}/build" 2>&1 | tee "${NORMAL_LOG}"

if grep -E "warning:|error:" "${STRICT_LOG}" "${NORMAL_LOG}" >/dev/null; then
  fail "SwiftPM emitted warning or error output"
fi
...
echo "✅ AppStateMachine verification passed"
