...
- **Repository path**: `./PackagesForReuse/IntegrationHelpers/AppAnalyticsPushNotificationsIntegration`
...
```swift
dependencies: [
    .package(path: "../PackagesForReuse/IntegrationHelpers/AppAnalyticsPushNotificationsIntegration")
],
targets: [
    .target(
        name: "YourTarget",
...
For Xcode, use **File → Add Package Dependencies… → Add Local…** and select `./PackagesForReuse/IntegrationHelpers/AppAnalyticsPushNotificationsIntegration`.
...
3. Add required `Sources/**/*.swift` and resources through `./scripts/migrate_packages_in_use_project.py --apply` or an equivalent project edit that preserves the `PackagesInUse/IntegrationHelpers/AppAnalyticsPushNotificationsIntegration` Xcode group.
