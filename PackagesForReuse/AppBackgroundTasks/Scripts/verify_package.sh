...
PACKAGE_NAME="AppBackgroundTasks"
...
cleanup() {
  rm -rf "${SCRATCH_ROOT}"
  rmdir "${WORKTREE_DIR}/WorktreeScratch" 2>/dev/null || true
}
trap cleanup EXIT
...
FORBIDDEN_IMPORTS=(
  ...
  AppImagePipeline AppDownloads AppUploads AppRemoteAssets AppTaskQueue
  AppRateLimiter AppStateMachine AppPagination AppFormValidation AppValidationCore
  AppInputFormatting AppDateTime AppNumberFormatting AppHaptics AppAccessibilitySupport
  AppReviewPrompt AppEmptyStateKit AppOnboarding AppURLSafety AppDeepLinking
)
...
if grep -R "String(describing: *error)\|localizedDescription\|@unchecked Sendable\|stablePrivacyHash\|raw telemetry\|bodyText\|Authorization\|Cookie" -n Sources Package.swift README.md PackageContract.md Docs 2>/dev/null; then
  fail "forbidden source/privacy/concurrency pattern found"
fi

BAD_TEMP_PATTERN="/t""mp"
BAD_ENV_PATTERN="TMP""DIR"
BAD_SCRATCH_WORDING="external scratch"" path"
if grep -R "${BAD_TEMP_PATTERN}\|${BAD_ENV_PATTERN}\|${BAD_SCRATCH_WORDING}" -n Sources Tests Package.swift README.md PackageContract.md Docs Scripts 2>/dev/null; then
  fail "temporary-directory wording or disallowed scratch wording is forbidden"
fi

rm -rf "${SCRATCH_ROOT}"
mkdir -p "${SCRATCH_ROOT}"

swift test --jobs 1 --scratch-path "${TEST_SCRATCH_PATH}" -Xswiftc -strict-concurrency=complete
